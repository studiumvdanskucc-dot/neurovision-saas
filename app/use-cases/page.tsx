...
function UseCaseArt({ type }: { type: string }) {
  ...
  if (type === "shelf-art") return <div className="use-case-art shelf-art amazon-shelf"><header><b>shop</b><input aria-label="Product search" value="sparkling water" readOnly/><span>Search</span></header><section>{["LIME","PURE","BERRY","CITRUS"].map((name,index)=><div className={index===1?"winner":""} key={name}><i/><strong>{name}</strong><small>12-pack · 4.6 ★</small>{index===1&&<><em>ANALYSED</em><u/></>}</div>)}</section><aside>Predicted shelf attention <b>87</b></aside></div>;
  if (type === "packaging-art") return <div className="use-case-art packaging-art soda-pack"><span><small>NV / SPARK</small><b>VOLT</b><em>YUZU + LIME</em><u>330 ml</u></span><i/><div><b>Brand</b><span>81%</span><b>Flavour</b><span>68%</span></div></div>;
  return <div className="use-case-art generation-art remake-art"><div><small>Before · original</small><span><b>MOVE<br/>DIFFERENT.</b></span></div><b>→</b><div className="remade"><small>After · AI-remade</small><span><b>BUILT<br/>TO MOVE.</b></span></div><em>Hierarchy +18%</em><u>Headline clarified · product enlarged · CTA restored</u></div>;
}
